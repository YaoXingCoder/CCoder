/*************************************************************************
    ...
    >   15.更改为非阻塞的监听, 方式1 通过设置recv的第四位参数
 ************************************************************************/
        ...
        for(int i = 0; i < nready; ++i) {
            ...
            if (rdfd == socketfd) {
                ...
                if (eventsArr[i].events & EPOLLIN ){
                    ...
                    printf("first recv:>\n");
                    int recvNum = recv(rdfd, recvLine, MAX_LINE, MSG_DONTWAIT); // 第一次读
                    printf("%s\n", recvLine);
                    ...
                    printf("second recv:>\n");
                    recvNum = recv(rdfd, recvLine, MAX_LINE, MSG_DONTWAIT); // 第二次读
                    if (recvNum == -1) { perror("recv"); }
                    printf("%s\n", recvLine);

                    // 给客户端回显消息
                    int sendNum = send(rdfd, recvLine, recvNum, MSG_DONTWAIT); 
