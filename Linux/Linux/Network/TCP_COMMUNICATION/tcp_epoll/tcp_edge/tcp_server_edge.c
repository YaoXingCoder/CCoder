    ...
    while(1) {
        ...
        for(int i = 0; i < nready; ++i) {
            ...
            if (rdfd == socketfd) {
                ...
                // 将acceptfd的设置为非阻塞, 通过fctnl获取其标志位信息
                // int flags = fcntl(acceptfd,F_GETFL, 0); // 先获取原有标志位信息
                // flags |= O_NONBLOCK; // 添加非阻塞标志位信息
                // fcntl(acceptfd, F_SETFL, flags); // 设置回去

                // epoll 对 acceptfd 进行监听
                memset(&ev, 0, sizeof(ev));
                // ev.events = EPOLLIN;
                // ev.events = EPOLLIN | EPOLLOUT; // 更改客户端的fd的监听事件为读|写事件
                ev.events = EPOLLIN | EPOLLOUT | EPOLLET; // 更改客户端为边缘触发
                ev.data.fd = acceptfd;
                ...
                if (eventsArr[i].events & EPOLLIN ){
                    ...
                    // 注释掉对读事件就绪的处理, 表示没有对该事件的处理
                    // 默认情况下, epoll对于事件的处理方式为水平出发
                    // epoll_wait 会一直返回, 通知处理该事件
                    // 接收客户端消息
                    memset(recvLine, 0, MAX_LINE);
                    int recvNum = recv(rdfd, recvLine, MAX_LINE, MSG_PEEK); // 更改MSG为 MSG_PEEK
                    ...
                    if (recvNum == 0) {
                        ...
                        recvNum = recv(rdfd, recvLine, sizeof(recvLine), 0); // 此处不设置MSG_PEEK, 从缓冲区取出数据                                                         // 如果缓冲区中的数据长度>20个字节, 进行处理
                                                                             // 给客户端回显消息
