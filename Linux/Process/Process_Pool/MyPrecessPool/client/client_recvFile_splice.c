...
int main (int argc, char* argv[]) {
    ...
    // 加入文件传输进度
    int len_persent = len_filecontent / 100;
    int lastRecvSize = 0;

    int recvSize = 0;
    while(recvSize < len_filecontent){
        ret = splice(clientfd, NULL, fds[1], NULL, 4096, 0);
        ret = splice(fds[0], NULL, fd, NULL, ret, 0);
        recvSize += ret;
        if ( recvSize - lastRecvSize > len_persent ) {
            printf("has recv %5.2f%%\r", (double)100 * recvSize / len_filecontent);
            fflush(stdout);
