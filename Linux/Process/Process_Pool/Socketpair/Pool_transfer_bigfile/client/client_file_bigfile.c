...
int main (int argc, char* argv[]) {
    ...
    while(totalSize < len_fileContent ){
        recv(clientfd, &len_recvSize, sizeof(len_recvSize), 0); // 先接受需要接受的信息
        printf("recv len_recvSize is %d\n", len_recvSize);
        ...
        int recvNum = recv(clientfd, t_file.buff, len_recvSize, 0); // 在接收文件内容信息
        printf("recvNum is %d\n", recvNum);
        if ( recvNum > 0 ) {
            totalSize += recvNum;
