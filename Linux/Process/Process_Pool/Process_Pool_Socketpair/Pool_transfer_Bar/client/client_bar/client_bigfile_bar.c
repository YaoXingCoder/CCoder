...
int main (int argc, char* argv[]) {
    ...
    while(totalSize < len_fileContent ){
        ...
        if ( recvNum > 0 ) {
            ...
            if ( totalSize - lastSize > segment ) {
                printf("has completed %5.2f%%\r", (double)100 * totalSize / len_fileContent); 
                fflush(stdout); // printf输出到终端是行缓冲, 遇到\n才会刷新输出, 所以需要注重刷新输出
                lastSize = totalSize;
            }
        }
    }

    printf("has completed 100.00%%\n");
